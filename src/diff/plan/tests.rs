...
mod tests {
    ...
    fn test_diff_add_phases() {
        ...
        // Collect the phase IDs and sort them for deterministic testing
        let mut added_ids: Vec<&str> = diff.iter().map(|d| match d {
            DiffResult::Add(phase) => phase.id.as_str(),
            _ => panic!("Expected Add operation"),
        }).collect();
        added_ids.sort();
        
        assert_eq!(added_ids, vec!["phase1", "phase2"]);
    }
